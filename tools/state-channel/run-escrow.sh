...
network_plan=${ESCROW_NETWORK_PLAN:-}
settings=$(node -e 'const network=require(process.argv[1]).loadNetwork();console.log(network.project+"\t"+network.peerPort)' "$root/tools/state-channel/escrow-network.js")
IFS=$'\t' read -r project peer_port <<< "$settings"
[[ -z "$(ss -H -ltn "sport = :$peer_port")" ]] || { echo "Port$peer_port is already used; do not replace its process" >&2; exit 1; }
mkdir "$out"
mkdir "$private"
out=$(realpath "$out")
...
mkdir "$out/source"
cp "$root/tools/state-channel/escrow-chain.js" "$root/tools/state-channel/escrow-peer.js" "$root/tools/state-channel/escrow-scenario.js" "$root/tools/state-channel/run-escrow.sh" "$root/tools/state-channel/chain-readiness.js" "$root/tools/state-channel/escrow-network.js" "$out/source/"
cp "$root/tools/state-channel/inspect-settlement.js" "$out/source/"
cp "$root/tools/state-channel/transaction-finality.js" "$out/source/"
if [[ -n "$network_plan" ]]; then
  cp "$network_plan" "$out/network-plan.json"
  export ESCROW_NETWORK_PLAN="$out/network-plan.json"
fi
...
docker inspect $(docker ps -q --filter "label=com.docker.compose.project=$project") \
...
if [[ -n "$network_plan" ]]; then base+=(--env ESCROW_NETWORK_PLAN=/evidence/network-plan.json); fi
containers=()
record_states() {
  result=$?
  trap - EXIT
...
run_chain() {
  phase=$1
  if [[ -n "$network_plan" ]]; then
    node "$out/source/escrow-network.js" "$out/$phase-protocol.json" "$out/network-plan.json" > "$out/$phase-protocol.log" 2>&1
  fi
...
run_chain settle
audit="ain-cert-escrow-$RUN_ID-audit"
docker create --name "$audit" "${base[@]}" "$image" /opt/ain-js/tools/state-channel/inspect-settlement.js /evidence /evidence/settlement-audit.json > "$out/audit-container-id.txt"
containers+=("$audit")
docker inspect "$audit" --format '{{json .HostConfig}}' > "$out/audit-limits.json"
docker start -a "$audit" > "$out/audit.log" 2>&1
